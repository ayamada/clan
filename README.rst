...
まず最初に、 al.clj と AndroidManifest.xml.in をいじって、
...
一通り開発ができたら、忘れずに、上記で変更した
al.clj と AndroidManifest.xml.in を元の状態に戻しておく事。
...
1. script/settings.sh のバージョン番号から、 ``-SNAPSHOT`` を除去
   ...
   -  必要なら、 sample/configure.in
      のサンプルアプリのバージョンも変更しておく
