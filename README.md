...
CLAN supply to an environment that develop android application
using libgdx by only clojure code.
You must understand to clojure and libgdx.

WARNING: This package version is 0.0.0-EXPERIMENTAL.
It cannot upgrade to future version 1.0.0 .
Please try to CLAN if you still OK.
...
バージョン 1.0.0 とします。
この 0.0.0 から 1.0.0 へのアップデートは非常に困難ですが、
それでもいい人は試してみてください。
...
- You must write specialized clojure-code for Android
  (typical clojure-code work better on PC, but it work slowly on Android)
...
- package/projname/.gitignore に、 CLAN本体を除外する設定を記述。
    - この .gitignore も spread_current_dir.sh によって生成されます。
- package/projname/ に、作りたいアプリ本体の各ファイルを設置。
    - これらのファイルも spread_current_dir.sh によって大体生成されます。
- package/ に、 `package/projname/` で生成されたリリース版実行バイナリを
 ...
 2. 必要に応じて、layout/*.in android/AndroidManifest.xml.in 等を編集
    - `*.in` というのは、autoconfによる書き換えを行う元ファイル。
      とりあえず、以下だけ把握していればok。
        ...
        - 例えば `hoge.xml.in` から `hoge.xml` が、同じディレクトリに生成される
        - ファイル内の `@HOGE@` のような部分のみが書き換えられる
        - 後述の `make clean` 系を実行すると、生成されたファイルも消える
 ...
 1. まだapk署名用の鍵を作ってなければ、作成します。
    - コンソールから以下のコマンドを実行します。
      質問されるので適切に入力し、パスワード等も決めてください。
        ...
        keytool -genkey -keystore path/to/NAME.keystore -validity 36500 -alias NAME
...
- 詳細については、 `clan/info/src/main/java/jp/ne/tir/clan/Info.java.in`
  を確認してください。しかし実際に使う可能性があるのは
  `Info/debug`, `Info/buildNumber`, `Info/BuildDate` ぐらいでしょう。
...
- サンプルを展開する
- うっかり再展開してしまわないように、一時的に clan/sample/ を空にしておく
...
- この手順の問題点として、サンプルアプリ部分のこまめなコミットができない
  ...
  それによって一時的な履歴を取るようにして対応する。
    - DropboxFilter.cfj を適切に設定し、中間ファイルを保存しないようにする
...
上記の通りに開発を行い、gitに保存したところまで進めておく事。
 ...
 6. git commit -m 'version X.Y.Z released'
 7. git push
...
- My app is too slow on android-real-machine.
    - enabling [*warn-on-reflection*][wor], and insert type specifier.
      it was used by [clojure-maven-plugin][cmp].
- android実機で超遅い
    - [*warn-on-reflection*][wor] を有効にして、型指定しまくる。
      [clojure-maven-plugin][cmp]からも使える。
        - しかしその結果、 `(let [^Hoge hoge (Hoge.)] ...)`
          のようなコードが量産される。なんというJava…

[wor]: http://clojure.org/java_interop#Java%20Interop-Type%20Hints
[cmp]: https://github.com/talios/clojure-maven-plugin#configuring-your-clojure-session

- Why cannot I compile `*.clj`, it was skipped.
    - set to encoding = utf-8. This is spec of clojure-maven-plugin probably.
...
- Where is save data for desktop jar/exe
    - it saves `C:\Users\{USERNAME}\.pref\` (for windows),
      or ~/.pref/ (other unix like OS).
      point is .prefs in home-directory by OS.
...
- [Clojure](http://clojure.org/)
    ...
    - [Clojure for Android](https://github.com/sattvik/clojure)
...
- [LibGDX](http://libgdx.badlogicgames.com/)
    ...
    - [Androidゲームプログラミング A to Z](http://www.impressjapan.jp/books/3113)([amazon](http://www.amazon.co.jp/dp/4844331132)): [Beginning Android Games](http://www.apress.com/9781430230427)の日本語訳版。良書。ただし本家の方は[2nd Edition](http://www.apress.com/9781430246770)が出てるが日本語訳版は初版ベース。
...
- [CLANによるandroidアプリ開発](http://doc.tir.ne.jp/devel/clan): 俺が書いてる。まだ途中。
...
- later
    - 開発版ビルドとリリース版ビルドの切り分け部分の改善
        ...
        - リリース版と開発版でAndroidManifest.xmlのuses-permissionの変更
          (要は開発時のみINTERNETとWRITE_EXTERNAL_STORAGEを有効にしたい的な)
    - collecting and documentation to know-how
      in clojure(for android), libgdx, android, neko
